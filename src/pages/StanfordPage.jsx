...
import { StanfordTemplate } from '../templates';
...
function StanfordPage() {
  ...
  const handleExportDOCX = async () => {
    ...
    try {
      await generateDOCX(formData, `${formData.fullName.replace(/\s+/g, '_')}_Resume_Stanford.docx`);
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans text-slate-900">
          ...
          <div className="mb-6 pb-6 border-b border-slate-100">
            ...
            <div className="text-center">
              <h1 className="text-xl sm:text-2xl font-bold text-slate-800 flex items-center justify-center gap-2">
                <FileText className="text-red-600" aria-hidden="true" />
                Stanford Template
              </h1>
              <p className="text-slate-500 text-sm mt-1">Modern & Professional design</p>
              {saveStatus && (
      ...
      <div className="hidden md:flex w-full md:w-7/12 lg:w-2/3 bg-slate-200 h-screen flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 flex justify-center">
          {activeTab === 'preview' && <StanfordTemplate formData={formData} />}
          {activeTab === 'markdown' && (
            <CodeView 
      ...
      <div className="md:hidden fixed bottom-4 right-4 z-50">
        <Link
          to="/"
          className="flex items-center gap-2 px-4 py-3 bg-red-600 text-white rounded-full shadow-lg hover:bg-red-700 transition-colors focus:outline-none focus:ring-2 focus:ring-red-500 focus:ring-offset-2"
          aria-label="Back to template selection"
        >
