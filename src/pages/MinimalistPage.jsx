...
import { MinimalistTemplate } from '../templates';
...
function MinimalistPage() {
  ...
  const handleExportPDF = async () => {
    ...
    const result = await generatePDF(formData, `${formData.fullName.replace(/\s+/g, '_')}_Resume_Minimalist.pdf`, 'minimalist');
  ...
  const handleExportDOCX = async () => {
    ...
    try {
      await generateDOCX(formData, `${formData.fullName.replace(/\s+/g, '_')}_Resume_Minimalist.docx`);
    ...
    <div className="min-h-screen bg-slate-100 flex flex-col md:flex-row font-sans text-slate-900">
        ...
        <div className="p-4 sm:p-6">
          <div className="mb-6 pb-6 border-b border-slate-100">
            ...
            <div className="text-center">
              <h1 className="text-xl sm:text-2xl font-bold text-slate-800 flex items-center justify-center gap-2">
                <FileText className="text-emerald-600" aria-hidden="true" />
                Minimalist Template
              </h1>
              <p className="text-slate-500 text-sm mt-1">Clean & Elegant design</p>
              {saveStatus && (
          ...
          <div className="mb-6 flex flex-wrap gap-2">
            <button
              ...
              disabled={isSaving}
              className="flex items-center gap-1 px-3 py-2 text-xs bg-emerald-600 hover:bg-emerald-700 text-white rounded-lg transition-colors disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
            ...
            <button
              ...
              className="flex items-center gap-1 px-3 py-2 text-xs bg-teal-600 hover:bg-teal-700 text-white rounded-lg transition-colors focus:outline-none focus:ring-2 focus:ring-teal-500 focus:ring-offset-2"
      ...
      <div className="hidden md:flex w-full md:w-7/12 lg:w-2/3 bg-slate-200 h-screen flex-col">
        ...
        <div className="flex-1 overflow-y-auto p-4 md:p-8 flex justify-center">
          {activeTab === 'preview' && <MinimalistTemplate formData={formData} />}
          {activeTab === 'markdown' && (
            <CodeView 
      ...
      <div className="md:hidden fixed bottom-4 right-4 z-50">
        <Link
          to="/"
          className="flex items-center gap-2 px-4 py-3 bg-emerald-600 text-white rounded-full shadow-lg hover:bg-emerald-700 transition-colors focus:outline-none focus:ring-2 focus:ring-emerald-500 focus:ring-offset-2"
          aria-label="Back to template selection"
        >
